...
The classifier uses `claude-sonnet-4-6` via multimodal vision. The evaluation compares Claude's structured JSON output against manually assigned ground-truth labels across five attributes: `garment_type`, `style`, `material`, `occasion`, and `location_context`.
...
This is the most uncertain attribute in the evaluation because it measures Claude's inference of _geographic design tradition_ from visual cues — a subtle and partially non-visual judgment.
...
3. **Single-model evaluation**: Only `claude-sonnet-4-6` was evaluated. Comparing against `claude-haiku-4-6` (cheaper, faster) or `claude-opus-4-6` (more capable) would provide a useful model quality vs. cost trade-off analysis.
